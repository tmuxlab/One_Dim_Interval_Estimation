GSpsimulate = function(estimate, roopj, estimationMethod, thresholdRule, thresholdValue, directory_path)
{
  tn <- length(estimate)
  timeindex <- 1:tn
  x <- c(0, timeindex)
  y <- rev(estimate); y <- c(0, y)
  ...
  GSp <- tryCatch(
    {
      nls(y ~ a * exp(b * x),
          start = list(a = 1, b = -0.0005),
          algorithm = "port",
          lower = list(a = 0.0005, b = -100),
          upper = list(a = 1000,   b = -0.0000005))
    },
    error = function(e) {
      fit_ok <<- FALSE
      y_pos <- pmax(y, 1e-8)
      co <- coef(lm(log(y_pos) ~ x))
      list(a = exp(co[[1]]), b = min(co[[2]], -0.0000005))
    }
  )
  
  if (isTRUE(fit_ok)) {
    a_hat <- as.numeric(coef(GSp)[["a"]])
    b_hat <- as.numeric(coef(GSp)[["b"]])
  } else {
    a_hat <- GSp$a
    b_hat <- GSp$b
  }
  ...
  get_nhpp_realization <- function(sita, beta, tn){
    ...
    if (length(A)) {
      B <- A; LL <- length(A)
      for (i in 1:LL) A[i] <- tn - B[LL + 1 - i]
    }
    A
  }
  
  # --- 本体：出力形式は元のまま（arrivals: NAパディング / counts: m×tn） ---
  brks <- 0:tn
  for (i in 1:row){
    ...
    # ヒストは t_samples から、端点含め right=FALSE
